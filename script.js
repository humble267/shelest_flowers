...
const buildSubcategoryCard = (category) => {
  ...
  const imageMarkup = image
    ? `<img class="catalog-group-card__img" src="${escapeHtml(image)}" alt="${escapeHtml(category.title)}" loading="lazy" />`
    : "";
...
const buildProductCard = (product) => {
  ...
  return `
    <article class="catalog-product${isAvailable ? "" : " is-unavailable"}">
      <a class="catalog-product__media${product.image ? "" : " is-empty"}" href="${productUrl}">
        ${
          discount > 0
      ...
      <div class="catalog-product__body">
        ...
        <div class="catalog-product__actions">
          <a class="catalog-product__details" href="${productUrl}">Детальніше</a>
          ${buyControl}
        </div>
      </div>
    </article>
...
const getProductLead = (product, category) => {
  if (category) {
    return `Композиція з категорії “${category.title}”, доступна для замовлення через Telegram.`;
  }

  return "Оберіть зручний спосіб замовлення та переходьте до оформлення через Telegram.";
};

const buildProductDetail = (product, category) => {
  const discount = Math.max(0, Math.round(product.discount));
  const isAvailable = isAvailableEntity(product.available);
  const currentPrice =
    discount > 0 ? getDiscountedPrice(product.price, discount) : product.price;
  const buyControl = isAvailable
    ? `<a class="product-detail__button" href="${getTelegramUrl(product)}" target="_blank" rel="noreferrer">Купити</a>`
  ...
  return `
    <section class="product-detail">
      ...
      <div class="product-detail__panel">
        ...
        ${
          category
            ? `<p class="product-detail__category">${escapeHtml(category.title)}</p>`
            : ""
        }
...
const renderProductPage = async () => {
  ...
  try {
    ...
    if (eyebrow) {
      eyebrow.textContent = category ? category.title : "SHELYST FLOWERS";
    }
    ...
    if (lead) {
      lead.textContent = getProductLead(product, category);
    }

    document.title = `${product.name} | SHELYST FLOWERS`;
    content.innerHTML = buildProductDetail(product, category);
    bindPageTransitions(content);
    applyImageFallbacks(content);
  } catch (error) {
